...
/** ----------------------------------------------------------------
 * NearshoreMoroccoSection
 * – Premium layout + a11y-friendly animations
 * – Image skeletons + graceful fallback
 * – Clear stats, benefits, and 1-FAQ objection handling
 * – Uses /public assets (don’t prefix with "public/")
 * ---------------------------------------------------------------- */
    ...
    <div className={`${ratio} w-full overflow-hidden rounded-2xl bg-white relative`}>
      ...
      {ok ? (
        <img
          ...
          onError={() => setOk(false)}
          className={`h-full w-full ${fit} transition-opacity duration-300 ${
...
export default function NearshoreMoroccoSection({
  onBook,
  metrics,
  assets,
}) {
    ...
    >
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-2">
          {/* Left: map visual card with floating chips */}
          <m.figure
          ...
          >
            ...
            <div className="pointer-events-none absolute left-4 sm:left-6 top-4 sm:top-6 inline-flex items-center gap-2 rounded-full bg-white/85 px-2.5 sm:px-3 py-1 text-[11px] sm:text-xs font-medium text-sky-700 ring-1 ring-sky-100 backdrop-blur">
              <Clock className="h-4 w-4" /> {M.overlapHours || overlapHours} CET/UK overlap
            </div>
            <div className="pointer-events-none absolute right-4 sm:right-6 bottom-4 sm:bottom-6 inline-flex items-center gap-2 rounded-full bg-white/85 px-2.5 sm:px-3 py-1 text-[11px] sm:text-xs font-medium text-emerald-700 ring-1 ring-emerald-100 backdrop-blur">
              <Plane className="h-4 w-4" /> {M.flight || flight}
            </div>
          </m.figure>
          ...
          >
            ...
            <m.ul variants={group} className="grid gap-4 sm:grid-cols-3">
              {[
                { k: M.savingRange || savingRange, v: "Cost efficiency" },
                { k: M.overlapHours || overlapHours, v: "CET/UK overlap" },
                { k: M.languages || languages, v: "Multilingual" },
              ].map(({ k, v }) => (
                <m.li
                  key={v}
                  variants={fadeUp}
        ...
        <m.div
          ...
          whileInView="show"
          viewport={{ once: true, amount: 0.25 }}
          className="mx-auto mt-10 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-4"
        >
          {[
            {
              ...
              icon: HandCoins,
              title: "Cost savings",
              text: `Typically ${M.savingRange || savingRange} vs Western Europe with senior talent.`,
              img: A.cost,
            },
            {
              ...
              icon: Clock,
              title: "Time-zone fit",
              text: `Daily ${M.overlapHours || overlapHours} overlap for stand-ups, pairing & reviews.`,
              img: A.timezone,
            },
            {
              ...
              text: `${M.languages || languages} for stakeholder alignment.`,
              img: A.languages,
            },
            {
              iconBg: "bg-emerald-50 text-emerald-600",
              ...
              text: "NDA, VPN/SOC controls, PII handling, ISTQB talent.",
              img: A.security,
            },
          ].map((c) => (
            <m.article
              ...
              variants={fadeUp}
              whileHover={{ y: -3 }}
              className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm transition hover:shadow-md"
            >
              ...
              <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 bg-gray-50 p-1 sm:p-2">
                <img
                  src={c.img}
                  alt={c.title}
                  className="mx-auto max-h-32 w-auto object-contain sm:max-h-40 md:max-h-50"
                  loading="lazy"
                />
              </div>
            </m.article>
        ...
        {/* One-click objection handling (FAQ) with animation */}
        <div className="mx-auto mt-10 max-w-4xl">
          {(() => {
            function FAQItem({ question, answer, defaultOpen = false }) {
              ...
              return (
                <m.article
                  ...
                  className="group mb-3 rounded-2xl border border-slate-200 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur hover:shadow-md"
                >
                  <button
                    onClick={() => setOpen((v) => !v)}
                    aria-expanded={open}
                    className="flex w-full items-center justify-between gap-3 sm:gap-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 rounded-xl"
                  >
                    <div>
                    ...
                    <m.span
                      ...
                      className="inline-grid h-8 w-8 sm:h-9 sm:w-9 shrink-0 place-items-center rounded-lg border border-slate-200 bg-white text-slate-700 shadow-sm"
                    >
            ...
            return (
              <div role="list" aria-label="Frequently asked questions" className="[&>article:last-child]:mb-0">
                <FAQItem
                  question="How do we collaborate day-to-day?"
                  answer={
                    <>
                      We schedule ceremonies inside the {M.overlapHours || overlapHours} window and keep pipelines,
                      dashboards, and PR templates aligned with your toolchain for fast feedback and fewer context switches.
                ...
                <FAQItem
                  ...
                  answer={
                    <>
                      NDA by default, VPN/SOC controls with least-privilege access, and strict PII handling. We can mirror
                      your SSO and enforce device posture checks for added control.
                    </>
                  }
                />
                <FAQItem
                  ...
                  answer={
                    <>
                      A typical ramp is 2–3 weeks: discovery & access (days 1–3), automation plan & env setup (week 1),
                      first test suites & CI wiring (week 2), coverage acceleration (week 3+). We share a 90-day roadmap on day 5.
                    </>
                  }
                />
              </div>
